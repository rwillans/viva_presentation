...
## Chapter One: Theoretical Background, Data and Estimation Issues
...
## Chapter Two: Scale Economies derived via a long-run translog cost function

Prior estimates - Positive scale economies up to 230 beds [@azevedoCostEffectsHospital2014] (Portuguese data), to 370 beds [@frechResolvingImpasseHospital1995] (US Data), or at least 400 beds [@wilsonNonparametricAnalysisReturns2004] (non-parametric method on US Data)

@scottInvestigatingHospitalEfficiency1995 only UK study I am aware of - did not go on to estimate from derived cost function. Increased data available now make this method feasible

Multiproduct translog used with inputs including outpatient care, elective and non-elective admitted care, diagnostic tests and A&E attendances.

Scale economies computed as inverse of the sum of individual cost-output elasticities - i.e. does a 1% increase in all outputs entail a proportionately higher or lower percentage increase in costs.

::: {.notes}
Chapter Two is the first analysis chapter proper, and estimates scale economies via a long-run translog cost function.

Most recently direct estimation of the limits to scale economies has been carried out by Azvedo and , and Frech ), xxx(ref) but as mentioned before most literature is dated and non for the uk. Scott et al. tried but were unsatisfied with their estimated function and the data available at the time.

Outputs modelled include ambulatory or outpatient care, admitted and emergency care, unbundled diagnostic tests. Scale economies are computed using the method originally defined by Baumol et al () Main result is the finding of small but positive economies of scale up to the level of approximately 1,100 beds.

Also notable that scale economies were lower in London and SE. In hindsight this makes intuitive sense, as proportionate increases in outputs would cost more than other areas, due to London weighting for nursing and other Agenda for Change staff, and perhaps a higher price level for non-pay expenditure (reflected by the dummy variable for London based trusts). This is consistent with the finding in Asmild and Hollingsworth (1321) that hospital costs can be affected by location, and that studies which do not attempt to model or account for different local price levels may be missing a source of variation.
...
## Chapter Three: Scale Economies derived via a long-run quadratic cost function
...
## Chapter Four: Estimation via a short-run cost function and envelope approach
...
To deal with this critique the suggestion is to deal with it in two steps. First to estimate a short-run cost function, proxying capital with beds. Then solve this and define optimal beds in at each level of output. By substituting this expression back into the short term cost function we obtain a long-run function assuming optimal capital levels at each output level.
The results from this approach using a translog are perhaps as expected. The cost-efficient optimal number of beds was found to be slightly lower than the observed values, suggesting that trusts wer overcapitalised. When optimal capital values were assumed, economies of scale rose, particularly at the upper end, so no limit was observed.
...
## Chapter Five: Scope Economies in Major Specialty Groupings
...
Finally, the last chapter discusses and concludes the results from the individual essays. It argues that as a result of this thesis, we can update estimates from old studies, suggesting that technology has moved on, and there is no evidence that diseconomies of scale now apply before 1,200 beds at least, and possibly further dependent on the particular method used. 
It also shows the effects different methodological assumptions can have, with some evidence for divergence between quadratic and translog functional forms. Future studies may wish to use both or at least consider that observed variation in results may be a feature of such forms.
Many studies also lack data on input prices, which are often assumed to have no effect where there are national pay or consumable procurement. This study includes input prices where able, and is able to demonstrate a cost function difference between high cost and low cost areas.
Policy Contributions
Policymakers can take note of the findings 
There may be small-scale benefits from increasing output amongst smaller hospitals, at least up to 1,200 beds and possibly further. These savings are unlikely to justify full mergers, but ought to be borne in mind when planning care provision.
There are lower scale economies in high-cost areas around London. It may be that any changes to healthcare supply in London or similar high cost areas take a different form to those in other areas of the country.
Finally, Consideration of the configuration of hospital services ought to consider effects of scope. This thesis only lightly touches on scope economies, but shows some evidence that ‘generalist’ specialties, and general surgery in particular has benefits from colocation with other activities, and that a base level of such healthcare ought to be supplied in each hospital.
